...
### 3 列表对象
列表对象的可选编码分别是：ziplist 和 quicklist（3.2 版本前是 linkedlist）。

#### 3.1 ziplist 编码的列表对象
ziplist 编码的列表对象使用压缩列表作为底层实现，每个压缩列表节点（entry）保存了一个列表元素。

我们执行以下 RPUSH 命令，服务器将会创建一个如图 9 所示结构的列表对象作为 numbers 键的值：
```
127.0.0.1:7379> RPUSH numbers 1 "three" 5
(integer) 3
127.0.0.1:7379> OBJECT ENCODING numbers
"quicklist"
```
![图 9 - ziplist 编码的列表对象](https://raw.githubusercontent.com/zibinli/blog/master/Redis/_v_images/20190523201925224_22719.png)
